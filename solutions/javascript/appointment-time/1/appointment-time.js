...
export function updateAppointment(timestamp, options) {
  ...
  for (let key in options) {
    switch(key) {
      case 'year':
        newDate.setFullYear(options[key]);
        break;

      case 'month':
        newDate.setMonth(options[key]);
        break;

      case 'date':
        newDate.setDate(options[key]);
        break;

      case 'hour':
        newDate.setHours(options[key]);
        break;

      case 'minute':
        newDate.setMinutes(options[key]);
        break;
  ...
  return getAppointmentDetails(newDate.getTime());
}
